<?php 
    ...
    $conn = getConnection();
    $totalSql = "SELECT COUNT(*) as total FROM services";
    $totalStmt = $conn->prepare($totalSql);
    $totalStmt->execute();
    $totalServices = $totalStmt->fetch(PDO::FETCH_ASSOC)['total'];
                            ...
                            <tbody style="font-size: 14px; color: #444;">
                                <?php 
                                    ...
                                    foreach($services as $index => $service){
                                        $serviveCardTitle = json_decode($service['specialization_card_title'], TRUE);
                                        $serviceCardDescription  = json_decode($service['specialization_card_description'], TRUE);
                                ...
                                ?>
                                 <tr>
                                    ...
                                    <td><img src="<?= BASE_URL . htmlspecialchars($service['service_icon']) ?>" alt="Icon"></td>
                                    <td><img src="<?= BASE_URL . htmlspecialchars($service['service_image']) ?>" alt="Banner"></td>
                                    <td><?= htmlspecialchars(substr($service['service_main_description'], 0, 200)) ?>...</td>
                                    <!-- <td><?= htmlspecialchars($service['specialization_heading']) ?></td> -->
                                    <!-- <td><?= htmlspecialchars($service['specialization_sub_heading']) ?></td> -->
                                    <td><?= htmlspecialchars($service['specialization_description']) ?></td>
                                    <td><?php if(!empty($serviveCardTitle) && is_array($serviveCardTitle)){
                                            echo htmlspecialchars(implode(" , ", $serviveCardTitle));
                                    }?></td>
                                    <!-- <td><?= htmlspecialchars($service['created_at']) ?></td> -->
                                    <td>
                                        <a href="/admin/services/edit?editId=<?= $service['id'] ?>" class="btn btn-sm btn-outline-primary me-2" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="/admin/service/delete?id=<?= $service['id'] ?>" class="btn btn-sm btn-outline-danger" title="Delete" onclick="return confirm('Are you sure you want to delete this service?')">
